    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        if (ep != nullptr) {
            ...
            status = metal_matmul_apply_epilogue(
                ctx, outs[i], ep_buffer, N * M, ep_cfgs[i].feature_dim, ep_cfgs[i].bias_scalar, ep
            );
            [ep_buffer release];
...
static marmot_error_t metal_matmul_qkv_run_quantized_route_b(
    ...
    id<MTLComputePipelineState> matmul_pipeline = metal_pipeline_get(ctx, kernel_name_to_use);
