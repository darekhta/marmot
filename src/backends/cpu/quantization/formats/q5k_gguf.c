...
static marmot_error_t q5_k_quantize_block(const float *values, uint32_t count, void *block_out, const void *params);
static marmot_error_t q5_k_dequantize_block(const void *block, uint32_t count, float *values_out, const void *params);

static const marmot_quant_traits_t q5_k_traits = {
    .kind = MARMOT_QUANT_KIND_Q5_K,
    .name = "Q5_K",
    .block_size = CPU_QUANT_FORMAT_BLOCK_VALUES(MARMOT_QUANT_KIND_Q5_K),
    .block_bytes = CPU_QUANT_FORMAT_BLOCK_BYTES(MARMOT_QUANT_KIND_Q5_K),
    .weight_bits = 5,
    .has_zero_point = true,
    .requires_calibration = false,
    .layout = CPU_QUANT_FORMAT_LAYOUT(MARMOT_QUANT_KIND_Q5_K),
    .compute_params = nullptr,
    .quantize_block = q5_k_quantize_block,
    .dequantize_block = q5_k_dequantize_block,
    .vec_dot_block = nullptr,
};

MARMOT_REGISTER_QUANT_SCHEME(q5_k_traits)

static marmot_error_t
q5_k_quantize_block(const float *values, uint32_t count, void *block_out, [[maybe_unused]] const void *params) {
    ...
    float mins[QK_K / 32];
    marmot_q5_k_block_t *block = (marmot_q5_k_block_t *)block_out;
    ...
    for (int j = 0; j < QK_K / 32; ++j) {
        ...
        scales[j] = make_qkx2_quants(
            32, 31, block_values + 32 * j, weights, L + 32 * j, &mins[j], Laux, -0.5f, 0.1f, 15, false
        );
...
static marmot_error_t
q5_k_dequantize_block(const void *block_in, uint32_t count, float *values_out, [[maybe_unused]] const void *params) {
    if (block_in == nullptr || values_out == nullptr || count == 0) {
        return MARMOT_ERROR_INVALID_ARGUMENT;
    }

    const uint32_t elems = qk_clamp_elems(count);
    const marmot_q5_k_block_t *block = (const marmot_q5_k_block_t *)block_in;
    ...
    for (size_t j = 0; j < QK_K && written < elems; j += 64) {
        ...
        for (int l = 0; l < 32 && written < elems; ++l, ++written) {
            uint8_t vh = (qh[l] & u1) ? 16 : 0;
            values_out[written] = d1 * ((ql[l] & 0xF) | vh) - m1;
        }
        for (int l = 0; l < 32 && written < elems; ++l, ++written) {
            uint8_t vh = (qh[l] & u2) ? 16 : 0;
            values_out[written] = d2 * (((ql[l] >> 4) & 0xF) | vh) - m2;
        }
        u1 <<= 2;
        u2 <<= 2;
        ql += 32;
...
marmot_error_t cpu_quantize_q5_k(const void *device_ctx, const marmot_tensor_t *input, marmot_tensor_t *output) {
    ...
    marmot_error_t err = require_k_traits(MARMOT_QUANT_KIND_Q5_K, &traits, "Q5_K traits not registered");
