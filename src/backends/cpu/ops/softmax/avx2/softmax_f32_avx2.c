...
static inline float softmax_find_max_avx2(const float *data, size_t n) {
    ...
    float max_val = softmax_horizontal_max_m256(max_vec);
...
static inline float softmax_exp_sum_avx2(const float *x, float *out, size_t n, float max_val) {
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        __m256 vexp = softmax_exp_avx2(_mm256_sub_ps(vx, max_vec));
        _mm256_storeu_ps(out + i, vexp);
        sum_vec = _mm256_add_ps(sum_vec, vexp);
    }

    float sum = softmax_horizontal_sum_m256(sum_vec);
    for (; i < n; i++) {
...
static inline void softmax_normalize_avx2(float *data, size_t n, float inv_sum) {
...
static inline void softmax_two_pass_avx2(const float *x, float *out, size_t n) {
    softmax_accum_f32_t acc = softmax_accumulate_f32(x, n);
    float inv_sum = acc.sum_exp > 0.0f ? 1.0f / acc.sum_exp : 0.0f;
    __m256 max_vec = _mm256_set1_ps(acc.max_val);
    ...
    for (; i + 8 <= n; i += 8) {
        __m256 vx = _mm256_loadu_ps(x + i);
        __m256 vexp = softmax_exp_avx2(_mm256_sub_ps(vx, max_vec));
        vexp = _mm256_mul_ps(vexp, inv_vec);
        _mm256_storeu_ps(out + i, vexp);
    }
...
static marmot_error_t softmax_f32_avx2(
    ...
    if (inner_stride == 1) {
        ...
        for (size_t i = 0; i < outer_size; i++) {
            ...
            if (use_two_pass) {
                softmax_two_pass_avx2(x_row, out_row, softmax_size);
            } else {
                float max_val = softmax_find_max_avx2(x_row, softmax_size);
                float sum = softmax_exp_sum_avx2(x_row, out_row, softmax_size, max_val);
                softmax_normalize_avx2(out_row, softmax_size, 1.0f / sum);
            }
        }
    } else {
...
const cpu_softmax_traits_t cpu_softmax_f32_avx2_traits = {
    .dtype = MARMOT_DTYPE_FLOAT32,
    .impl_kind = SOFTMAX_IMPL_AVX2,
    .fn = softmax_f32_avx2,
    .impl_name = "f32_avx2",
};
SOFTMAX_REGISTER_TRAITS(cpu_softmax_f32_avx2_traits)
