...
static inline void cpu_embedding_copy_f32_neon(const float *src, float *dst, size_t dim, float scale) {
    size_t i = 0;
...
static inline void cpu_embedding_copy_u16_neon(const uint16_t *src, uint16_t *dst, size_t dim) {
    size_t i = 0;
    for (; i + 8 <= dim; i += 8) {
...
marmot_error_t cpu_embedding_gather_neon(const void *device_ctx, const marmot_embedding_gather_desc_t *desc) {
    ...
    for (size_t idx = 0; idx < token_count; ++idx) {
        ...
        if (f32_path) {
            cpu_embedding_copy_f32_neon((const float *)src_row_bytes, (float *)out_row_bytes, dim, scale);
        } else {
            cpu_embedding_copy_u16_neon((const uint16_t *)src_row_bytes, (uint16_t *)out_row_bytes, dim);
        }
    }
